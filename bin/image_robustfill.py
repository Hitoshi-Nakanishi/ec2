...
class Image_RobustFill(nn.Module):
    def __init__(self, target_vocabulary, hidden_size=512, embedding_size=128, cell_type="LSTM", input_size=(3, 256, 256)):
        ...
        self.conv3 = nn.Conv2d(16, 16, kernel_size=(3, 3), padding=(1, 1), stride=(1, 1))
        self.conv4 = nn.Conv2d(16, 16, kernel_size=(3, 3), padding=(1, 1), stride=(1, 1))
        ...
        self.img_feat_to_embedding = nn.Sequential(
            nn.Linear(16 * 16 * 16, 64), nn.ReLU(), nn.Linear(64, 64), nn.ReLU(), nn.Linear(64, self.hidden_size)
        )
    ...
    def _run(self, inputs, target=None, mode="sample", n_samples=None):
        ...
        def attend(i, j, h):
            ...
            linear_transformed_img = transformed_img.view(
                -1, transformed_img.size(1) * transformed_img.size(2) * transformed_img.size(3)
            )
...
if __name__ == "__main__":
    ...
    train_loader = torch.utils.data.DataLoader(
        datasets.Omniglot("../vhe/data", download=True, background=True, transform=omni_transforms),
    ...
    test_loader = torch.utils.data.DataLoader(
        datasets.Omniglot("../vhe/data", download=True, background=False, transform=omni_transforms),
        batch_size=batch_size,
        shuffle=True,
        **kwargs
    )
    ...
    for i, batch in enumerate(train_loader):
        ...
        if i % 10 == 0:
            print(
                "Iteration %d" % (i), "Score %3.3f" % score, "(%3.3f seconds per iteration)" % ((time.time() - start) / (i + 1))
            )
