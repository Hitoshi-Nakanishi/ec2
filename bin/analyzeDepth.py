...
def loadCheckpoint(r):
    ...
    domain = {"logo": "LOGO", "tower": "Towers", "text": "Text", "list": "List", "rational": "Regression"}[domain]
    result = loadPickle(r)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--recognition", nargs="+", default=[], help="List of checkpoints that use the full model.")
    parser.add_argument("--generative", nargs="+", default=[], help="List of checkpoints that use the no-recognition lesion.")
    ...
    if arguments.every:
        ...
        for mode in ["MEAN", "SIZE", "MAX"]:
                ...
                for domain, data in results.items():
                    for i, y, d in data:
                        ...
                        if results is fullResults:
                            ...
                            # "#ef5675"
                            c = (factor * 239.0 / 239.0, factor * 86.0 / 239.0, factor * 117.0 / 239.0)
                        C.append(c)
            ...
            plot.xlabel({"MAX": "Max depth", "MEAN": "Avg. depth", "SIZE": "Library size"}[mode], fontsize=FONTSIZE)
            ...
            plot.savefig(f"figures/depthVersusAccuracy_revision_{mode}.png", bbox_inches="tight")
    ...
    for mode in ["MAX", "MEAN", "SIZE"]:
        ...
        plot.savefig(f"figures/depthVersusAccuracy_mainPaper_{mode}.eps", bbox_inches="tight")
        plot.figure()
        legend = plot.legend(handles=handles, handletextpad=0.05, bbox_to_anchor=(1, 0.5), loc="center", ncol=1)
        f = legend.figure
        f.canvas.draw()
