...
def padSearchTimes(result, testingTimeout, enumerationTimeout):
    result.testingSearchTime = [ts + [testingTimeout] * (result.numTestingTasks - len(ts)) for ts in result.testingSearchTime]
    result.searchTimes = [ts + [enumerationTimeout] * (len(result.taskSolutions) - len(ts)) for ts in result.searchTimes]
...
def getLikelihood(likelihood, result, task, iteration):
    ...
    if likelihood == "marginal":
        if arguments.testingLikelihood:
            return lse([e.logPosterior + testingRegexLikelihood(task, e.program) for e in frontier])
        else:
    ...
    if likelihood == "task":
        if arguments.testingLikelihood:
            return lse([testingRegexLikelihood(task, e.program) + e.logPrior for e in frontier])
        else:
...
def getTestingLikelihood(likelihood, result, iteration):
    ...
    totalCharacters = sum(len(s) for t in testingTasks for _, s in regexHeldOutExamples(t))
    print("Total number of characters in testing tasks is", totalCharacters)
    return sum(getLikelihood(likelihood, result, task, iteration) for task in testingTasks) / totalCharacters


def getTrainingLikelihood(likelihood, result, iteration):
    totalCharacters = sum(len(s) for task in result.taskSolutions.keys() for _, s in task.examples)
    return sum(getLikelihood(likelihood, result, task, iteration) for task in result.taskSolutions.keys()) / totalCharacters
...
def showSynergyMatrix(results):
    ...
    print(
        "Of the", len(results), "checkpoints that you gave me, here is a matrix showing the overlap between the tasks solved:"
    )
    ...
    if len(results) == 3:
        ...
        print(int(len(everSolved[0] - everSolved[1] - everSolved[2]) / len(everSolved[0]) * 100 + 0.5), end="%")
        print()
...
def plotECResult(
    ...
    if onlyTime:
        ...
        if arguments.ylabel is not None:
            ...
            if numTasks:
                ylabel = "Num: %s Solved%s" % ("Training" if showTraining else "Test", " (solid)" if showSolveTime else "")
            else:
                ylabel = "%% %s Solved%s" % ("Training" if showTraining else "Test", " (solid)" if showSolveTime else "")
    ...
    for result, p, color in zip(results, parameters, colors):
        ...
        if arguments.caching and os.path.exists(cachingFileKey):
            ...
            if testingTimeout is not None:
                result.testingSearchTime = [[t for t in ts if t <= testingTimeout] for ts in result.testingSearchTime]
            ...
            if likelihood is None:
                if showTraining:
                    if numTasks:
                        ...
                        ys = [100.0 * t / float(len(result.taskSolutions)) for t in result.learningCurve[:iterations]]
                else:
                    if cutoff is None:
                        if numTasks:
                            ys = [len(t) for t in result.testingSearchTime[:iterations]]
                        else:
                            ys = [100.0 * len(t) / result.numTestingTasks for t in result.testingSearchTime[:iterations]]
                    else:
                        ys = getCutOffHits(result, cutoff)[:iterations]
            else:
                ys = [
                    (getTrainingLikelihood if showTraining else getTestingLikelihood)(likelihood, result, iteration)
        ...
        if showSolveTime:
            ...
            if interval and result is results[0]:
                ...
                a2.fill_between(
                    xs,
                    [percentile(ts, 0.75) if timePercentile else mean(ts) + standardDeviation(ts) for ts in times],
                    [percentile(ts, 0.25) if timePercentile else mean(ts) - standardDeviation(ts) for ts in times],
                    facecolor=color,
                    alpha=0.2,
                )

    if averageColors:
        plotCommands_solve = {kl: averageCurves(curves) for kl, curves in plotCommands_solve.items()}
        plotCommands_time = {kl: averageCurves(curves) for kl, curves in plotCommands_time.items()}
        if solveAxis:
            for (color, ls), (xs, ys, es) in plotCommands_solve.items():
                solveAxis.fill_between(
                    xs, [y - e for y, e in zip(ys, es)], [y + e for y, e in zip(ys, es)], facecolor=color, alpha=0.4
                )
            for (color, ls), (xs, ys, es) in plotCommands_solve.items():
        ...
        plotCommands_solve = {kl: medianCurves(curves) for kl, curves in plotCommands_solve.items()}
        ...
        if solveAxis:
            for (color, ls, xs, ys) in shuffled(
                [(color, ls, xs, ys) for (color, ls), cs in plotCommands_solve.items() for xs, ys in cs]
            ):
        ...
        if timeAxis:
            for (color, ls, xs, ys) in shuffled(
                [(color, ls, xs, ys) for (color, ls), cs in plotCommands_time.items() for xs, ys in cs]
            ):
        ...
        for n in range(len(arguments.baselines) // 2):
            ...
            if name.startswith("style"):
                ...
                if len(name) > 1:
                    ...
                    style = "-"
                plot.axhline(bl, -0.5, iterations, color=color, lw=3, linestyle=style, zorder=-1.0)
            else:
                plot.axhline(bl, -0.5, iterations, color="k", lw=3, zorder=-1.0)
                solveAxis.text(iterations, bl, name, ha="left", va="center", fontweight="bold", fontsize=TICKFONTSIZE)
    ...
    if labels is not None:
        a1.legend(
            ...
            fancybox=True,
            shadow=True,
            handles=[mlines.Line2D([], [], color=color, ls="-", label=label) for label, color in usedLabels],
        )
    f.tight_layout()
    if export:
        plot.savefig(export)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--names", "-n", type=str, default="", help="comma-separated list of names to put on the plot for each checkpoint"
    )
    ...
    parser.add_argument("--interval", default=False, action="store_true", help="Should we show error bars for synthesis times?")
    parser.add_argument(
        "--testingTimeout",
        default=None,
        type=float,
    ...
    parser.add_argument("--maxPercent", "-m", type=int, default=110, help="Maximum percent for the percent hits graph")
    ...
    parser.add_argument("--noTime", default=False, action="store_true", help="Do not show solve time.")
    parser.add_argument(
        "--onlyTime",
        default=False,
        action="store_true",
    ...
    parser.add_argument("--epochFrequency", default=1, type=int, help="Frequency with which to show epoch markers.")
    parser.add_argument(
        "--averageColors",
        default=False,
        action="store_true",
    ...
    parser.add_argument("--alpha", default=1.0, type=float, help="Transparency of plotted lines")
    parser.add_argument("--likelihood", type=str, choices=["maximum", "task", "marginal", "MAP"], default=None)
    parser.add_argument("--cutoff", type=str, choices=["bigram", "unigram", "gt"], default=None)
    parser.add_argument("--testingLikelihood", default=False, action="store_true")
    parser.add_argument(
        "--goodPrior", default=False, action="store_true", help="Do not update priors when doing cutoffs and likelihoods"
    )
    parser.add_argument("--baselines", default=[], nargs="+")
    parser.add_argument("--palette", "-c", default=False, action="store_true")
    parser.add_argument(
        "--median", default=False, action="store_true", help="Plots the median and 25/75 percentile of hits over time"
    )
    parser.add_argument("--width", "-w", default=5, type=float)
    parser.add_argument("--height", "-H", default=2.5, type=float)
    parser.add_argument(
        "--caching", default=False, action="store_true", help="try to cashe the calculation of the curves on disk"
    )
    parser.add_argument(
        "--numTasks", default=False, action="store_true", help="only show the number of tasks instead of percentile"
    )
    ...
    if arguments.palette:
        ...
        mapping = dict(
            zip(
                ["teal", "orange", "purple", "cyan", "c1", "c2", "c3", "c4", "c5", "c6"],
                ["#003f5c", "#ffa600", "#ef5675", "#7a5195", "#003f5c", "#444e86", "#955196", "#dd5182", "#ff6e54", "#ffa600"],
            )
        )
