...
def plotTimeMetrics(resultPaths, experimentNames, outlierThreshold, metricsToPlot, timesArg, export=None):
    ...
    for j, path in enumerate(resultPaths):
        ...
        for k, metricToPlot in enumerate(metricsToPlot):
            ...
            plotEmbeddingWithLabels(
                times_and_metrics, taskNames, title, os.path.join(export, domain, "labels_" + export_name), xlabel, ylabel
            )
...
def plotEmbeddingWithLabels(embeddings, labels, title, exportPath, xlabel=None, ylabel=None, colorLabeling=None):
    ...
    if colorLabeling is "text":
        ...
        legend = plot.legend(handles=patches, frameon=True, loc="upper center", bbox_to_anchor=(0.5, -0.02), ncol=5)
    ...
    if colorLabeling is "list":
        ...
        legend = plot.legend(
            handles=patches, frameon=True, loc="upper center", bbox_to_anchor=(0.5, -0.02), ncol=4, fontsize=15
        )
...
def plotLabeledImages(embeddings, images, labels, title, exportPath, xlabel=None, ylabel=None, colorLabeling=None):
    ...
    legend = plot.legend(handles=patches, frameon=True, loc="upper center", bbox_to_anchor=(0.5, -0.02), ncol=1, fontsize=15)
    ...
    for index, (x, y, i) in enumerate(imageLabels):
        ab = AnnotationBbox(OffsetImage(i, zoom=zoom), (x, y), xycoords="data", frameon=False)
...
def frontierMetric(frontiers):
    primitives = {p for f in frontiers for e in f.entries for p in [e.program] if p.isInvented}
    ...
    for j, path in enumerate(resultPaths):
        ...
        for k, metricToCluster in enumerate(metricsToCluster):
            ...
            if metricToCluster == "frontier":
                taskMetrics = [f.expectedProductionUses(result.grammars[-1]) for f in taskMetrics]
            ...
            clusteredTaskMetrics = random_restart_tsne(taskMetrics, perplexity=tsnePerplexity, lr=tsneLearningRate, restarts=10)
            title = title or (
                "Metric: %s, Domain: %s, Experiment: %s, Iteration: %d" % (metricToCluster, domain, experimentName, iterations)
            )

            if labelWithImages or labelsAndImages:
                images = {}
                for i, task in enumerate(
                    sorted(filter(lambda mt: isinstance(mt, Task), recognitionTaskMetrics.keys()), key=lambda task: task.name)
                ):  # Enumerate in same order as sorted tasks.
                    if domain == "tower":
                        recognitionTaskMetrics[task]["taskImages"] = task.getImage(pretty=not labelsAndImages)
                    if "taskImages" not in recognitionTaskMetrics[task] and domain == "rational":
                        recognitionTaskMetrics[task]["taskImages"] = task.features
                    if "taskImages" not in recognitionTaskMetrics[task] and domain == "logo":
                ...
                if not labelsAndImages:
                    plotEmbeddingWithImages(
                        ...
                        title,
                        os.path.join(
                            export, domain, experimentName + metricToCluster + "_iters_" + str(iterations) + "_tsne_images.png"
                        ),
                    )
                else:
                    plotLabeledImages(
                        ...
                        os.path.join(
                            export,
                            domain,
                            experimentName + metricToCluster + "_iters_" + str(iterations) + "_tsne_labeledimages.png",
                        ),
                    )
                ...
                plotEmbeddingWithLabels(
                    ...
                    title,
                    os.path.join(
                        export, domain, experimentName + metricToCluster + "_iters_" + str(iterations) + "_tsne_labels.png"
                    ),
                )
...
def DPGMM(metricsDict, groundTruths):
    ...
    metricNorms = (taskMetrics * taskMetrics).sum(1) ** 0.5
    taskMetrics = taskMetrics / np.reshape(metricNorms, (metricNorms.shape[0], 1))
    dpgmm = mixture.BayesianGaussianMixture(n_components=len(groundTruths), covariance_type="full").fit(taskMetrics)
    ...
    for j, path in enumerate(checkpoints):
        ...
        for task in sorted(recognitionTaskMetrics.keys(), key=lambda task: formattedName("frontier", task)):
            ...
            if "expectedProductionUses" in recognitionTaskMetrics[task]:
                ...
                testExpectedProductions[task] = recognitionTaskMetrics[task]["frontier"].expectedProductionUses(
                    result.grammars[-1]
                )
...
def plotHeatMap(cm, classes, title="Confusion matrix", cmap=plot.cm.Blues, exportPath=None):
    """
...
def getExpectedProductionSimilarities(checkpoints, ordering, applySoftmax):
    ...
    for j, path in enumerate(checkpoints):
        ...
        allEPs.append(
            [
                (trainNames, trainExpected, getSimilarities(trainExpected, applySoftmax)),
...
def logo_tower_NamesToLabels(listNames, nameMapping):
    ...
    for name in listNames:
        ...
        for label, pretty in nameMapping:
            if (arguments.programColors is None and label in name) or (arguments.programColors is not None and label == name):
                labels.append(pretty)
                foundName = True
                break
        if not foundName:
    ...
    labels = [prettyNames.index(label) if label in prettyNames else None for label in labels]
    return labels
...
def gallery(array, ncols=6):
    ...
    result = (
        array.reshape(nrows, ncols, height, width, intensity).swapaxes(1, 2).reshape(height * nrows, width * ncols, intensity)
    )
    return result
...
def similarityAnalysis(
    ...
    for j, path in enumerate(checkpoints):
        ...
        for similarityMetric in similarityMetrics:
            (trainMetrics, trainMetricSims), (testMetrics, testMetricSims) = getSimilarityMetrics(
...
if __name__ == "__main__":
    ...
    if arguments.exportTaskTimes:
        exportTaskTimes(arguments.checkpoints, arguments.experimentNames, arguments.times, arguments.export)
