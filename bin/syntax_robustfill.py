...
class SyntaxCheckingRobustFill(nn.Module):
    def __init__(
        self, input_vocabularies, target_vocabulary, hidden_size=512, embedding_size=128, cell_type="LSTM", max_length=25
    ):
        """
        ...
        if cell_type == "GRU":
            ...
            self.encoder_cells = nn.ModuleList(
                ...
                + [
                    nn.GRUCell(input_size=self.v_inputs[i] + 1 + self.hidden_size, hidden_size=self.hidden_size, bias=True)
        ...
        self.As = nn.ModuleList(
            [nn.Bilinear(self.hidden_size, self.hidden_size, 1, bias=False) for i in range(self.n_encoders)]
        )
    ...
    def _run(self, inputs, target=None, mode="sample", n_samples=None, vocab_filter=None):
        ...
        if vocab_filter is not None:
            vocab_mask = self.t.new(
                [[v in V for v in self.target_vocabulary] + [True] for V in vocab_filter]
            ).byte()  # True for STOP
        ...
        if self.no_inputs:
            ...
            inputs_scatter = [
                [
                    Variable(
                        self._zeros(max_length_inputs[i][j], batch_size, self.v_inputs[i] + 1).scatter_(
                            2, inputs[i][j][:, :, None], 1
                        )
                    )
        ...
        if target is not None:
            target_scatter = Variable(
                self._zeros(max_length_target, batch_size, self.v_target + 1).scatter_(2, target[:, :, None], 1)
            ...
            for j in range(n_examples):
                ...
                for k in range(max_length_inputs[i][j]):
                    ...
                    if k + 1 < max_length_inputs[i][j]:
                        active[k + 1, :] = active[k, :] * (inputs[i][j][k, :] != self.v_inputs[i])
                    h = self._cell_get_h(state)
                    hs.append(h[None, :, :])
        ...
        if self.no_inputs:
            decoder_states = [self._decoder_get_init(batch_size=batch_size)]  # TODO: learn seperate one of these
        else:
            ...
            for j in range(1 if self.no_inputs else n_examples):
                if mode == "score":
                    ...
                    target_char_scatter = Variable(
                        self._zeros(batch_size, self.v_target + 1).scatter_(1, target[k, :, None], 1)
                    )
        ...
        for i in range(self.n_encoders):
            ...
            for j in range(len(inputsss[0])):
                ...
                for k in range(len(inputs)):
                    ...
                    try:
                        if len(s) > 0:
                            t[: len(s), k] = torch.LongTensor([self.input_vocabularies_index[i][x] for x in s])
                    except KeyError:
                        import pdb
    ...
    def _targetToTensor(self, targets):
        ...
        for i in range(len(targets)):
            s = targets[i]
            if len(s) > 0:
                t[: len(s), i] = torch.LongTensor([self.target_vocabulary_index[x] for x in s])
        return t
            ...
            for target, score, decoder_states, syntax_decoder_state, active in beam:
                if not any(active == True):
                    if len(new_beam) < beam_size:
                        new_beam.append(
                            (target, score, decoder_states, syntax_decoder_state, active)
        ...
        for j in range(1 if self.no_inputs else n_examples):
            ...
            p_aug = (
                ...
                if self.no_inputs
                    ...
                    [h, self.attend_for_beam(self.n_encoders, j, h, H, attention_mask, max_length_inputs, batch_size)], 1
                )
    ...
    def _run_second_half(
        self, k, logsoftmax, target, token, score, decoder_states, syntax_decoder_state, active, batch_size, n_examples
    ):
