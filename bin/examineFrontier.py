...
def prettyRegex(preg):
    return (
        ...
        .replace("[0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ!\"#$%&'()*+,-./:;<=>?@[\\]^_`{|}~ \t]", ".")
    )
...
if __name__ == "__main__":
    ...
    for task in tasks:
        ...
        def examineProgram(entry):
            ...
            testing_likelihood = sum(preg.match(testingString) for _, testingString in testingExamples)
            ground_truth_testing = sum(gt_preg.match(testingString) for _, testingString in testingExamples)
        ...
        programSamples = [p.visit(ConstantVisitor(task.str_const)).evaluate([])(pre.String("")) for p in programSamples]
        ...
        posterior = [
            (e.logPosterior, e.program.visit(ConstantVisitor(task.str_const)).evaluate([])(pre.String("")))
            for e in frontier.normalize()
        ]
        testingExamples = [te for _, te in testingExamples]
        print("testingExamples", testingExamples)
        testingLikelihood = lse([lp + sum(r.match(te) for te in testingExamples) for lp, r in posterior])
        trainingExamples = ["".join(example[1]) for example in task.examples]
        print("trainingExamples", trainingExamples)
        trainingLikelihood = lse([lp + sum(r.match(te) for te in trainingExamples) for lp, r in posterior])
