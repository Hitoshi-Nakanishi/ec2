...
def test_task(m, task, timeout):
            ...
            if p not in failed_cands:
                if "STRING" in str(p):
                    ...
                    if len(task.stringConstants) == 0:
                        ...
                        ci = Text.ConstantInstantiateVisitor([[cc for cc in sc] for sc in task.stringConstants], sample=False)
                        ll = min(
                            task.logLikelihood(pp, timeout=0.1 if arguments.domain != "rational" else None)
                    ...
                    if not p.canHaveType(task.request):
                        ...
                        try:
                            ...
                            if arguments.sampleLikelihood:
                                ...
                                dataLikelihood = sum(regex.match("".join(y)) for _, y in task.examples)
                                logPrior = g.logLikelihood(task.request, p)
                                frontier.append(FrontierEntry(p, logPrior=logPrior, logLikelihood=dataLikelihood))
    ...
    if arguments.sampleLikelihood:
        return lse([math.log(frequency) + testingRegexLikelihood(task, regex) for regex, frequency in sampleFrequency.items()])
...
if __name__ == "__main__":
    ...
    if arguments.domain == "text":
        ...
        g = Grammar.uniform(dreamcoder.domains.logo.logoPrimitives.primitives, continuationType=turtle)
    ...
    if not arguments.test:
        ...
        if arguments.domain in ["tower", "rational", "logo"]:
            ...
            m = SyntaxCheckingRobustFill(input_vocabularies=input_vocabularies, target_vocabulary=target_vocabulary)
        ...
        if arguments.domain == "regex":
            ...
            totalCharacters = sum(len(s) for t in test for _, s in regexHeldOutExamples(t))
            if arguments.taskLikelihood:
                print(
                ...
                print("average likelihood of held out examples (normalized per character)", total_likelihood / totalCharacters)
        else:
            print("final score:")
