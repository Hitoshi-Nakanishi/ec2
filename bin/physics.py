...
def makeTasks(namePrefix, vignettes):
        ...
        for i in range(len(v.trajectories)):
            ...
            for t in range(1, len(v)):  # for each time step
                ...
                xs = [v.trajectories[i][t - 1].json()] + [
                    v.trajectories[j][t - 1].json() for j in range(len(v.trajectories)) if j != i
                ]
    ...
    tasks = []
    for suffix, examples in [("velocity", velocityExamples), ("delta", deltaVelocityExamples)]:
        t = Task("%s-%s" % (namePrefix, suffix), arrow(*([tobject] * (outputObjects) + [tvector])), [])
        t.specialTask = (
            "physics",
            {
                "parameterPenalty": 20,
