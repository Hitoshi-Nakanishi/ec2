...
def parallelMap(numberOfCPUs, f, *xs, chunksize=None, maxtasksperchild=None, memorySensitive=False, seedRandom=False):
...
def callCompiled(f, *arguments, **keywordArguments):
    ...
    p = subprocess.Popen(["pypy3"] + pypyArgs + [compiled_driver_file], stdin=subprocess.PIPE, stdout=subprocess.PIPE)
...
def diffuseImagesOutward(imageCoordinates, labelCoordinates, d, maximumRadius=2.5, minimumRadius=1.5):
    import numpy as np
