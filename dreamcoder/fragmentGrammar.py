...
class FragmentGrammar(object):
    ...
    def _logLikelihood(self, context, environment, request, expression):
        ...
        if request.isArrow():
            ...
            return self._logLikelihood(context, [request.arguments[0]] + environment, request.arguments[1], expression.body)
        ...
        possibleVariables = float(int(any(isinstance(candidate, Index) for _, _, _, candidate in candidates)))
        possibleUses = {candidate: 1.0 for _, _, _, candidate in candidates if not isinstance(candidate, Index)}
        ...
        for f, xs in expression.applicationParses():
            for candidateLikelihood, newContext, tp, production in candidates:
                ...
                if production.isIndex:
                    ...
                    try:
                        newContext, fragmentType, variableBindings = Matcher.match(newContext, production, f, len(xs))
    ...
    def expectedUses(self, frontiers):
        ...
        return sum(math.exp(l - z) * u for z, frontier in zip(zs, likelihoods) for l, u in frontier)

    def insideOutside(self, frontiers, pseudoCounts):
        uses = self.expectedUses(frontiers)
        return FragmentGrammar(
            log(uses.actualVariables + pseudoCounts) - log(max(uses.possibleVariables, 1.0)),
            [
                (log(uses.actualUses.get(p, 0.0) + pseudoCounts) - log(uses.possibleUses.get(p, 0.0) + pseudoCounts), t, p)
    ...
    def jointFrontiersLikelihood(self, frontiers):
        return sum(
            lse([entry.logLikelihood + self.logLikelihood(frontier.task.request, entry.program) for entry in frontier])
            for frontier in frontiers
        )
    ...
    def jointFrontiersMDL(self, frontiers, CPUs=1):
        return sum(
            parallelMap(
                CPUs,
                lambda frontier: max(
                    entry.logLikelihood + self.logLikelihood(frontier.task.request, entry.program) for entry in frontier
                ),
                frontiers,
            )
        )
    ...
    def toGrammar(self):
        return Grammar(self.logVariable, [(l, q.infer(), q) for l, t, p in self.productions for q in [defragment(p)]])
    ...
    def normalize(self):
        ...
        return FragmentGrammar(self.logVariable - z, [(l - z, t, p) for l, t, p in self.productions])

    def makeUniform(self):
        return FragmentGrammar(0.0, [(0.0, p.infer(), p) for _, _, p in self.productions])
    ...
    def induceFromFrontiers(
        ...
        def restrictFrontiers():
            return parallelMap(CPUs, lambda f: bestGrammar.rescoreFrontier(f).topK(topK), frontiers)
        ...
        if aic is not POSITIVEINFINITY:
            ...
            while True:
                ...
                fragments = [
                    ...
                    if f not in bestGrammar.primitives and defragment(f) not in bestGrammar.primitives
                ]
                eprint("Proposed %d fragments." % len(fragments))

                candidateGrammars = [FragmentGrammar.uniform(bestGrammar.primitives + [fragment]) for fragment in fragments]
                ...
                bestGrammar.productions[-1] = (newPrimitiveLikelihood, concretePrimitive.tp, concretePrimitive)
        ...
        if False and any(productionUses.get(p, 0) < 0.5 for p in grammar.primitives if p.isInvented):
            uselessProductions = [p for p in grammar.primitives if p.isInvented and productionUses.get(p, 0) < 0.5]
