...
def helmholtzEnumeration(g, request, inputs, timeout, _=None, special=None, evaluationTimeout=None):
...
def backgroundHelmholtzEnumeration(tasks, g, timeout, _=None, special=None, evaluationTimeout=None):
    from pathos.multiprocessing import Pool

    requests = list({t.request for t in tasks})
    inputs = {r: list({tuplify(xs) for t in tasks if t.request == r for xs, y in t.examples}) for r in requests}
