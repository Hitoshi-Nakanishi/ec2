...
try:  # pypy will fail
    ...
    class TowerCNN(nn.Module):
        ...
        def featuresOfTask(self, t, t2=None):  # Take a task and returns [features]
            return self(t.getImage(), None if t2 is None else t2.getImage(drawHand=True))
        ...
        def featuresOfTasks(self, ts, t2=None):  # Take a task and returns [features]
            ...
            if t2 is None:
                ...
                t2 = np.array(
                    [
                        t.getImage(drawHand=True) if t else np.zeros((self.inputImageDimension, self.inputImageDimension, 3))
                        for t in t2
                    ]
                )
            else:
...
def main(arguments):
    ...
    g0 = Grammar.uniform({"new": new_primitives, "old": primitives}[arguments.pop("primitives")], continuationType=ttower)
