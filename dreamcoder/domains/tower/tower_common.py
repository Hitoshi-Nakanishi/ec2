...
def simulateWithoutPhysics(plan, ordered=True):
    ...
    def placeBlock(world, block):
        lowest = max(
            [lowestPossibleHeight(block)] + [overlap(block, other) for other in world if overlap(block, other) is not None]
        )
...
def renderPlan(
    ...
    def clear(x, y):
        for xp, yp, wp, hp in world:
            if x < xp + wp / 2.0 and x > xp - wp / 2.0 and y < yp + hp / 2.0 and y > yp - hp / 2.0:
                return False
        return True
