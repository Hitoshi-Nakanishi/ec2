...
def make_list_tasks(n_examples):
    ...
    for routine in lr.find(count=100):  # all routines
        ...
        if routine.is_parametric():
            ...
            for params in map(lambda values: dict(zip(keys, values)), product(range(6), repeat=len(keys))):
                try:
...
def make_list_bootstrap_tasks():
    ...
    lengthBootstrap = [
        ...
        Task("length int", arrow(tlist(tint), tint), [((l,), len(l)) for _ in range(10) for l in [randomList()]]),
        Task(
            "map length",
            arrow(tlist(tlist(tint)), tlist(tint)),
            [((xss,), [len(xs) for xs in xss]) for _ in range(10) for xss in [randomListOfLists()]],
        ),
    ]

    # Encourages learning of unfolding
    unfoldBootstrap = [
        Task("countdown", arrow(tint, tlist(tint)), [((n,), list(range(n + 1, 1, -1))) for n in range(10)]),
        Task("weird count", arrow(tint, tlist(tint)), [((n,), list(range(-n, 0, -1))) for n in range(-10, 0)]),
        Task(
            "take every other",
        ...
        Task(
            "drop last element",
            arrow(tlist(tint), tlist(tint)),
            [((l,), l[:-1]) for _ in range(10) for l in [[randint(0, 9) for _ in range(randint(2, 5))]]],
        ),
        # Task("suffixes", arrow(tlist(tint), tlist(tlist(tint))),
        #      [((l,), suffixes(l))
        #       for _ in range(10)
        #       for l in [randomList()]]),
        Task("range", arrow(tint, tlist(tint)), [((n,), list(range(n))) for n in range(10)]),
        Task("range inclusive", arrow(tint, tlist(tint)), [((n,), list(range(n + 1))) for n in range(10)]),
    ...
    arrayBootstrap = [
        Task(
            ...
            [((n, l), l[n]) for n in range(10) for l in [[randint(0, 9) for _ in range(randint(n + 1, n + 5))]]],
        ),
        ...
        Task(
            "1-index int",
            arrow(tint, tlist(tint), tint),
            [((n, l), l[n - 1]) for n in range(1, 11) for l in [[randint(0, 9) for _ in range(randint(n + 1, n + 4))]]],
        )
    ...
    sliceBootstrap = [
        Task(
            ...
            [((n, l), l[:n]) for n in range(10) for l in [[flip() for _ in range(randint(n, n + 5))]]],
        ),
        Task(
            "drop bool",
            arrow(tint, tlist(tbool), tlist(tbool)),
        ...
        Task(
            ...
            [((n, l), l[:n]) for n in range(10) for l in [[randint(0, 9) for _ in range(randint(n, n + 5))]]],
        ),
        Task(
            "drop int",
            arrow(tint, tlist(tint), tlist(tint)),
    ...
    foldBootstrap = [
        Task(
            "stutter",
            arrow(tlist(tint), tlist(tint)),
            [((l,), [z for x in l for z in [x, x]]) for _ in range(10) for l in [randomList()]],
        ),
        Task("sum", arrow(tlist(tint), tint), [((l,), sum(l)) for _ in range(10) for l in [randomList()]]),
        ...
        Task(
            "append constant 0", arrow(tlist(tint), tlist(tint)), [((l,), l + [0]) for _ in range(10) for l in [randomList()]]
        ),
    ]
    ...
    mapBootstrap = [
        Task(
            "map double",
            arrow(tlist(tint), tlist(tint)),
            [((l,), list(map(lambda n: n * 2, l))) for _ in range(10) for l in [randomList()]],
        ),
        Task(
            "map increment",
            arrow(tlist(tint), tlist(tint)),
            [((l,), list(map(lambda n: n + 1, l))) for _ in range(10) for l in [randomList()]],
        ),
        Task(
            "map negation",
            arrow(tlist(tint), tlist(tint)),
            [((l,), list(map(lambda n: 0 - n, l))) for _ in range(10) for l in [randomList()]],
        ),
    ...
    difficultMaps = [
        Task(
            "map quadruple",
            arrow(tlist(tint), tlist(tint)),
            [((l,), list(map(lambda n: n * 4, l))) for _ in range(10) for l in [randomList()]],
        ),
        Task(
            "map add 3",
            arrow(tlist(tint), tlist(tint)),
            [((l,), list(map(lambda n: n + 3, l))) for _ in range(10) for l in [randomList()]],
        ),
    ]
    ...
    filterBootstrap = [
        ...
        Task(
            "remove 0s",
            arrow(tlist(tint), tlist(tint)),
            [((xs,), [x for x in xs if x != 0]) for _ in range(10) for xs in [[randint(0, 3) for _ in range(5)]]],
        ),
        Task(
            "remove non-positives",
            arrow(tlist(tint), tlist(tint)),
            [((xs,), [x for x in xs if not (x > 1)]) for _ in range(10) for xs in [[randint(0, 3) for _ in range(5)]]],
        ),
    ]

    return lengthBootstrap + filterBootstrap + unfoldBootstrap + arrayBootstrap + foldBootstrap + mapBootstrap + zipBootstrap
...
def bonusListProblems():
    ...
    bonus = [
        Task(
            "pair reverse",
            arrow(tlist(tint), tlist(tint)),
            [((x,), [x[j + (1 if j % 2 == 0 else -1)] for j in range(len(x))]) for _ in range(5) for x in [randomList(10, 10)]],
        ),
        Task(
            "duplicate each element",
            arrow(tlist(tint), tlist(tint)),
            [((x,), [a for z in x for a in [z] * 2]) for _ in range(5) for x in [randomList(4, 6)]],
        ),
        Task(
...
def sortBootstrap():
    ...
    filterBootstrap = [
        ...
        Task(
            "remove > 1",
            arrow(tlist(tint), tlist(tint)),
            [((xs,), [x for x in xs if not (x > 1)]) for _ in range(10) for xs in [[randint(0, 5) for _ in range(7)]]],
        ),
    ]

    # Needed for selection sort
    minimumBootstrap = [
        Task("min2", arrow(tint, tint, tint), [((x, y), min(x, y)) for x in range(4) for y in range(4)]),
        Task("minimum of list", arrow(tlist(tint), tint), [((l,), min(l)) for _ in range(15) for l in [randomList()]]),
    ]
    ...
    appendBootstrap = [
        Task(
            ...
            [((x, y), x + y) for _ in range(10) for [x, y] in [[randomBooleanList(), randomBooleanList()]]],
        ),
        Task(
            "append int",
            arrow(tlist(tint), tlist(tint), tlist(tint)),
            [((x, y), x + y) for _ in range(10) for [x, y] in [[randomList(), randomList()]]],
        ),
    ]
    ...
    insertionBootstrap = [
        Task(
            ...
            arrow(tint, tlist(tint), tlist(tint)),
            [((x, l), [y for y in l if y >= x]) for _ in range(15) for x in [randint(0, 5)] for l in [randomList()]],
        ),
        Task(
            "filter less than",
            arrow(tint, tlist(tint), tlist(tint)),
    ...
    sortTask = [
        Task(
            ...
            arrow(tlist(tint), tlist(tint)),
            [((l,), list(sorted(l))) for _ in range(15) for l in [removeDuplicates(randomList())]],
        )
    ]

    slowSort = [
        Task("+1 maximum list", arrow(tlist(tint), tint), [((l,), max(l) + 1) for _ in range(15) for l in [randomList()]]),
        Task(
            "range +1 maximum list",
            arrow(tlist(tint), tlist(tint)),
            [((l,), list(range(max(l) + 1))) for _ in range(15) for l in [randomList()]],
        ),
    ]
