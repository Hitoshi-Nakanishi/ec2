...
def competeOnOneTask(checkpoint, task, CPUs=8, timeout=3600, evaluationTimeout=0.0005):
    if checkpoint.recognitionModel is not None:
        ...
        challengeFrontiers, times, bestSearchTime = recognizer.enumerateFrontiers(
            [task], CPUs=CPUs, maximumFrontier=1, enumerationTimeout=timeout, evaluationTimeout=evaluationTimeout
        )
    else:
...
def sygusCompetition(checkpoints, tasks):
    ...
    for t in tasks:
        for checkpoint in checkpoints:
            promise = workers.apply_async(competeOnOneTask, (checkpoint, t), {"CPUs": CPUs, "timeout": timeout})
...
def text_options(parser):
    parser.add_argument(
        "--showTasks", action="store_true", default=False, help="show the training test and challenge tasks and then exit"
    )
    parser.add_argument(
        "--trainChallenge",
        action="store_true",
...
def main(arguments):
    ...
    if arguments.pop("trainChallenge"):
        ...
        eprint(
            "Incorporating %d (50%%) challenge problems into the training set." % (len(challengeTrain)),
    ...
    ConstantInstantiateVisitor.SINGLE = ConstantInstantiateVisitor(
        list(map(list, list({tuple([c for c in s]) for t in test + train + challenge for s in t.stringConstants})))
    )
    ...
    baseGrammar = Grammar.uniform(
        ...
        + [
            ...
            if (p.name != "map" or haveMap) and (p.name != "unfold" or haveUnfold) and (p.name != "length" or haveLength)
        ]
    )
