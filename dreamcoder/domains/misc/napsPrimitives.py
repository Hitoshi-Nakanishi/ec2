...
# record
def _func(string):
    return lambda tp: lambda name: lambda vars1: lambda vars2: lambda stmts: [string, tp, name, vars1, vars2, stmts]
...
# lhs
def _if(tp):
    return lambda expr: lambda stmts1: lambda stmts2: ["if", tp, expr, stmts1, stmts2]  # TODO
...
def _invoke(tp):
    return lambda function_name: lambda exprs: ["invoke", tp, function_name, exprs]  # TODO, deal with fn_name and lists
