...
def _getspan(r1):
    return lambda i1: lambda b1: lambda r2: lambda i2: lambda b2: lambda string: string[
        ...
        if b1 == "End"
        else [m.start() for m in re.finditer(r1, string)][i1] : [m.end() for m in re.finditer(r2, string)][i2]
        if b2 == "End"
...
def _getspan_const(r1):
    return lambda i1: lambda b1: lambda r2: lambda i2: lambda b2: (
        defaultdict(int, {r1: i1 + 1 if i1 >= 0 else abs(i1), r2: i2 + 1 if i2 >= 0 else abs(i2)}),
...
def _gettoken_const(tp, i):
    return defaultdict(int, {tp: i + 1 if i >= 0 else abs(i)}), i + 1 if i >= 0 else abs(i)
...
def _concat2(expr1):
    return lambda expr2: lambda string: expr1(string) + expr2(string)  # More concats plz
...
def robustFillPrimitives(max_len=100, max_index=5):
    return (
        ...
        + [
            ...
            CPrimitive(
                "GetSpan", arrow(tregex, tindex, tboundary, tregex, tindex, tboundary, tsubstr), _getspan, _getspan_const
            ),  # TODO constraint
        ]
        + [
            # nestings
            CPrimitive("GetToken" + name + str(i), tnesting, _gettoken(tp, i), _gettoken_const(tp, i))
        ...
        + [
            CPrimitive("Replace_" + name1 + name2, tnesting, _replace(char1, char2), (defaultdict(int, {char1: 1}), 1))
        ...
        + [
            CPrimitive("GetUpTo" + name, tnesting, _getupto(reg), (defaultdict(int, {reg: 1}), 1))
            for name, reg in regexes.items()
        ]
        + [
            CPrimitive("GetFrom" + name, tnesting, _getfrom(reg), (defaultdict(int, {reg: 1}), 1))
            for name, reg in regexes.items()
        ]
        + [
            CPrimitive(
        ...
        + [
            # positions
            CPrimitive("position" + str(i), tposition, i, (defaultdict(int), i + 1 if i >= 0 else abs(i)))
