    ...
    class LearnedFeatureExtractor(RecurrentFeatureExtractor):
        ...
        def __init__(self, tasks, testingTasks=[], cuda=False):
            ...
            self.maximumLength = max(
                len(l) for t in tasks + testingTasks for xs, y in self.tokenize(t.examples) for l in [y] + [x for x in xs]
            )
...
def regex_options(parser):
    ...
    parser.add_argument("-H", "--hidden", type=int, default=256, help="number of hidden units")
    parser.add_argument(
        "--likelihoodModel",
    ...
    parser.add_argument("--topk_use_map", dest="topk_use_only_likelihood", action="store_false")
    ...
    parser.add_argument("--use_str_const", action="store_true", help="use string constants")
...
def main(args):
    ...
    extractor = {"learned": LearnedFeatureExtractor, "json": MyJSONFeatureExtractor}[args.pop("extractor")]
    ...
    productions = [
        (math.log(0.5 / float(n_base_prim)), prim) if prim.name not in specials else (math.log(0.10), prim)
