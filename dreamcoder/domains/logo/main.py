...
try:
    ...
    print("WARNING: Could not import torch. This is only okay when doing pypy compression.", file=sys.stderr)
...
try:
    ...
    print("WARNING: Could not import recognition. This is only okay when doing pypy compression.", file=sys.stderr)
...
def animateSolutions(allFrontiers):
    ...
    drawLogo(*programs, pretty=True, smoothPretty=True, resolution=128, animate=True, filenames=filenames)


def dreamFromGrammar(g, directory, N=100):
    if isinstance(g, Grammar):
        programs = [p for _ in range(N) for p in [g.sample(arrow(turtle, turtle), maximumDepth=20)] if p is not None]
    else:
        programs = g
    drawLogo(
        *programs,
...
try:
    ...
    class LogoFeatureCNN(nn.Module):
        ...
        def forward(self, v):
            ...
            reshaped = [
                floatOnlyTask[i : i + self.inputImageDimension] for i in range(0, len(floatOnlyTask), self.inputImageDimension)
            ]
...
def list_options(parser):
    parser.add_argument("--proto", default=False, action="store_true", help="Should we use prototypical networks?")
    parser.add_argument("--target", type=str, default=[], action="append", help="Which tasks should this try to solve")
    parser.add_argument("--reduce", type=str, default=[], action="append", help="Which tasks should this try to solve")
    parser.add_argument("--save", type=str, default=None, help="Filepath output the grammar if this is a child")
    parser.add_argument(
        "--prefix", type=str, default="experimentOutputs/", help="Filepath output the grammar if this is a child"
    )
    parser.add_argument("--dreamCheckpoint", type=str, default=None, help="File to load in order to get dreams")
    parser.add_argument("--dreamDirectory", type=str, default=None, help="Directory in which to dream from --dreamCheckpoint")
    parser.add_argument("--visualize", default=None, type=str)
    parser.add_argument("--cost", default=False, action="store_true", help="Impose a smooth cost on using ink")
...
def enumerateDreams(checkpoint, directory):
    ...
    frontiers = backgroundHelmholtzEnumeration(
        makeTasks(None, None), g, 100, evaluationTimeout=0.01, special=LogoFeatureCNN.special
    )()
    ...
    while len(frontiers) > 0:
        ...
        if fs:
            ...
            dreamFromGrammar([list(f.entries)[0].program for f in fs], f"{directory}/{MDL}")
        MDL += 1
...
def visualizePrimitives(primitives, export="/tmp/logo_primitives.png"):
    ...
    distances = [
        Program.parse(l) for l in ["logo_ZL", "logo_epsL", "(logo_MULL logo_epsL 2)", "(logo_DIVL logo_UL 2)", "logo_UL"]
    ]
    subprograms = [
        parseLogo(sp)
        for sp in [
    ...
    entireArguments = {
        "#(lambda (lambda (#(#(lambda (lambda (lambda (logo_forLoop $2 (lambda (lambda (logo_FWRT $2 $3 $0))))))) logo_IFTY) (logo_MULA (#(logo_DIVA logo_UA) $1) $0) (#(logo_MULL logo_UL) 3))))": [
            ...
            for xs in [("3", "1", "$0"), ("4", "1", "$0"), ("5", "1", "$0"), ("5", "3", "$0"), ("7", "3", "$0")]
        ]
    }
    specialDistances = {
...
def main(args):
    ...
    generator = ecIterator(
        baseGrammar, train, testingTasks=test, outputPrefix="%s/logo" % outputDirectory, evaluationTimeout=0.01, **args
    )
