...
def drawLogo(
    *programs, timeout=None, resolution=None, pretty=False, smoothPretty=False, filenames=[], animate=False, cost=False
):
    message = {}
    if pretty:
    ...
    if cost:
        ...
        response = [
            programResponse if isinstance(programResponse, str) else (programResponse["pixels"], programResponse["cost"])
            for programResponse in response
        ]
    else:
        response = [
            programResponse if isinstance(programResponse, str) else programResponse["pixels"] for programResponse in response
        ]
    if len(programs) == 1:
        return response[0]
    return response
...
def parseLogo(s):
    ...
    def command(k, environment, continuation):
        ...
        if k[0] == Symbol("move"):
            return Application(
                Application(Application(_move, expression(k[1], environment)), expression(k[2], environment)), continuation
            )
        if k[0] == Symbol("for") or k[0] == Symbol("loop"):
            ...
            return Application(Application(Application(_loop, b), Abstraction(Abstraction(body))), continuation)
        if k[0] == Symbol("embed"):
...
def manualLogoTask(name, expression, proto=False, needToTrain=False, supervise=False, lambdaCalculus=False):
...
def dSLDemo():
    ...
    def T(source):
        demos.append(manualLogoTask(str(len(demos)), source, lambdaCalculus="lambda" in source))
...
def manualLogoTasks():
    ...
    def T(name, source, needToTrain=False, supervise=False):
        tasks.append(manualLogoTask(name, source, supervise=supervise, needToTrain=needToTrain))
    ...
    T("right angle epsilon", "((move epsilonLength (/a 1a 4)) (move epsilonLength 0a))", needToTrain=True)
    ...
    for n, l in [(3, "(/d 1d 2)"), (4, "(/d 1d 3)")]:
        T("%d dashed lines of size %s" % (n, l), """(loop i %d (p (move 1d 0a)) (move %s 0a))""" % (n, l), needToTrain=n == 3)
    T(
        "broken circle",
        """
...
def demoLogoTasks():
    ...
    N = 1000
    programs = [p for _ in range(N) for p in [g0.sample(arrow(turtle, turtle), maximumDepth=20)] if p is not None]
    ...
    for n, t in enumerate(tasks):
        a = t.highresolution
        w = int(len(a) ** 0.5)
        scipy.misc.imsave("/tmp/logo%d.png" % n, np.array([a[i : i + w] for i in range(0, len(a), w)]))
    ...
    for t in dSLDemo():
        a = t.highresolution
        w = int(len(a) ** 0.5)
        scipy.misc.imsave("/tmp/logoDemo%s.png" % t.name, np.array([a[i : i + w] for i in range(0, len(a), w)]))
