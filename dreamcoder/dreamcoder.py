...
def ecIterator(
    ...
    if useRecognitionModel and featureExtractor is None:
        eprint("Warning: Recognition model needs feature extractor.", "Ignoring recognition model.")
    ...
    if message:
        message = " (" + message + ")"
    eprint("Running EC%s on %s @ %s with %d CPUs and parameters:" % (message, os.uname()[1], datetime.datetime.now(), CPUs))
    ...
    if addFullTaskMetrics:
        ...
        updateTaskSummaryMetrics(
            result.recognitionTaskMetrics, recognizer.taskGrammarLogProductions(tasks), "taskLogProductions"
        )
        updateTaskSummaryMetrics(result.recognitionTaskMetrics, recognizer.taskGrammarEntropies(tasks), "taskGrammarEntropies")
        updateTaskSummaryMetrics(
            result.recognitionTaskMetrics, result.recognitionModel.taskAuxiliaryLossLayer(tasks), "taskAuxiliaryLossLayer"
        )

        updateTaskSummaryMetrics(result.recognitionTaskMetrics, testingTimes, "heldoutTestingTimes")
        updateTaskSummaryMetrics(
            result.recognitionTaskMetrics, recognizer.taskGrammarLogProductions(testingTasks), "heldoutTaskLogProductions"
        )
        updateTaskSummaryMetrics(
            result.recognitionTaskMetrics, recognizer.taskGrammarEntropies(testingTasks), "heldoutTaskGrammarEntropies"
        )
        updateTaskSummaryMetrics(
        ...
        updateTaskSummaryMetrics(
            result.recognitionTaskMetrics, {f.task: f for f in trainFrontiers + testFrontiers if len(f) > 0}, "frontier"
        )
        SUFFIX = ".pickle"
        assert path.endswith(SUFFIX)
    ...
    for j in range(resume or 0, iterations):
        ...
        if (
            useRecognitionModel and biasOptimal and helmholtzRatio > 0 and all(str(p) != "REAL" for p in grammar.primitives)
            ...
            if useDSL or "helmholtzFrontiers" not in locals():
                helmholtzFrontiers = backgroundHelmholtzEnumeration(
                    tasks, grammar, enumerationTimeout, evaluationTimeout=evaluationTimeout, special=featureExtractor.special
                )
            else:
        ...
        result.taskSolutions = {f.task: f.topK(topK) for f in result.allFrontiers.values()}
        ...
        updateTaskSummaryMetrics(
            result.recognitionTaskMetrics, {f.task: f for f in result.allFrontiers.values() if len(f) > 0}, "frontier"
        )
...
def showHitMatrix(top, bottom, tasks):
    ...
    eprint("{: <13s}{: ^13d}{: ^13d}".format("top miss", len(bottomMiss & topMiss), len(bottom & topMiss)))
    eprint("{: <13s}{: ^13d}{: ^13d}".format("top hit", len(top & bottomMiss), len(top & bottom)))
...
def evaluateOnTestingTasks(
    ...
    updateTaskSummaryMetrics(result.recognitionTaskMetrics, {f.task: f for f in testingFrontiers if len(f) > 0}, "frontier")
...
def sleep_recognition(
    ...
    result.trainSearchTime = {tk: tm for tk, tm in ensembleRecognitionTimes[bestRecognizer].items() if tm is not None}
    ...
    updateTaskSummaryMetrics(
        result.recognitionTaskMetrics, result.recognitionModel.taskGrammarLogProductions(tasks), "taskLogProductions"
    )
    updateTaskSummaryMetrics(
        result.recognitionTaskMetrics, result.recognitionModel.taskGrammarEntropies(tasks), "taskGrammarEntropies"
    )
    if contextual:
        updateTaskSummaryMetrics(
            result.recognitionTaskMetrics, result.recognitionModel.taskGrammarStartProductions(tasks), "startProductions"
        )
    ...
    for bottomupFrontiers in ensembleFrontiers:
        for b in bottomupFrontiers:
            ...
            result.allFrontiers[b.task] = result.allFrontiers[b.task].combine(grammar.rescoreFrontier(b)).topK(maximumFrontier)
...
def consolidate(
    ...
    needToSupervise = {f.task for f in result.allFrontiers.values() if f.task.supervision is not None and f.empty}
    compressionFrontiers = [
        f.replaceWithSupervised(grammar) if f.task in needToSupervise else f for f in result.allFrontiers.values()
    ]
...
def commandlineArguments(
    ...
    parser.add_argument("-i", "--iterations", help="default: %d" % iterations, default=iterations, type=int)
    parser.add_argument(
        "-t", "--enumerationTimeout", default=enumerationTimeout, help="In seconds. default: %s" % enumerationTimeout, type=int
    )
    parser.add_argument(
        "-R",
        "--recognitionTimeout",
    ...
    parser.add_argument("-p", "--pseudoCounts", default=pseudoCounts, help="default: %f" % pseudoCounts, type=float)
    parser.add_argument("-b", "--aic", default=aic, help="default: %f" % aic, type=float)
    parser.add_argument("-l", "--structurePenalty", default=structurePenalty, help="default: %f" % structurePenalty, type=float)
    parser.add_argument("-a", "--arity", default=a, help="default: %d" % a, type=int)
    parser.add_argument("-c", "--CPUs", default=CPUs, help="default: %d" % CPUs, type=int)
    parser.add_argument(
        "--no-cuda",
        action="store_false",
        dest="cuda",
    ...
    parser.add_argument("-d", "--no-dsl", dest="useDSL", action="store_false", help="""Disable DSL enumeration and updating.""")
    parser.add_argument("--no-consolidation", dest="noConsolidation", action="store_true", help="""Disable DSL updating.""")
    parser.add_argument(
        "--testingTimeout",
        type=int,
        dest="testingTimeout",
    ...
    parser.add_argument("--testEvery", type=int, dest="testEvery", default=1, help="Run heldout testing every X iterations.")
    parser.add_argument(
        "--seed", type=int, default=0, help="Random seed. Currently this only matters for random batching strategies."
    )
    parser.add_argument(
        "--activation",
    ...
    parser.add_argument("--compressor", default=compressor, choices=["pypy", "rust", "vs", "pypy_vs", "ocaml", "memorize"])
    parser.add_argument(
        "--matrixRank",
    ...
    parser.add_argument("--mask", help="Unconditional bigram masking", default=False, action="store_true")
    parser.add_argument(
        "--biasOptimal",
    ...
    parser.add_argument(
        "--contextual", help="bigram recognition model (default is unigram model)", default=False, action="store_true"
    )
    parser.add_argument(
        "--clear-recognition",
...
def addTaskMetrics_(result, path):
    ...
    updateTaskSummaryMetrics(
        result.recognitionTaskMetrics, result.recognitionModel.auxiliaryPrimitiveEmbeddings(), "auxiliaryPrimitiveEmbeddings"
    )
    updateTaskSummaryMetrics(
        result.recognitionTaskMetrics, result.recognitionModel.taskAuxiliaryLossLayer(tasks), "taskAuxiliaryLossLayer"
    )
    updateTaskSummaryMetrics(
        result.recognitionTaskMetrics, result.recognitionModel.taskAuxiliaryLossLayer(everyTask), "every_auxiliaryLossLayer"
    )
    updateTaskSummaryMetrics(
        result.recognitionTaskMetrics,
    ...
    updateTaskSummaryMetrics(
        result.recognitionTaskMetrics, result.recognitionModel.taskGrammarLogProductions(tasks), "contextualLogProductions"
    )
    updateTaskSummaryMetrics(
        result.recognitionTaskMetrics,
    ...
    updateTaskSummaryMetrics(result.recognitionTaskMetrics, result.recognitionModel.taskHiddenStates(tasks), "hiddenState")
    updateTaskSummaryMetrics(
        result.recognitionTaskMetrics, result.recognitionModel.taskHiddenStates(everyTask), "every_hiddenState"
    )
    g = result.grammars[-2]  # the final entry in result.grammars is a grammar that we have not used yet
    updateTaskSummaryMetrics(
        result.recognitionTaskMetrics,
        {f.task: f.expectedProductionUses(g) for f in result.taskSolutions.values() if len(f) > 0},
