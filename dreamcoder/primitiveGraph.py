...
def graphPrimitives(result, prefix, view=False):
    ...
    age = {p: min(j for j, g in enumerate(result.grammars) if p in g.primitives) + 1 for p in primitives}
    ...
    def getName(p):
        ...
        for original, simplified in nameSimplification.items():
            simplification_ = simplification_.substitute(Primitive(original, None, None), Primitive(simplified, None, None))
        name[p] = "f%d" % len(name)
            ...
            with g.subgraph(name="cluster_%d" % o) as sg:
                ...
                for p in ordering[o]:
                    if str(p) in englishDescriptions:
                        thisLabel = '<<font face="boldfontname"><u>%s</u></font><br />%s>' % (
                            englishDescriptions[str(p)],
                            simplification[p],
                        )
                    else:
