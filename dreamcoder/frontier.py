...
class Frontier(object):
    ...
    def json(self):
        return {
            ...
            "task": str(self.task),
            "programs": [{"program": str(e.program), "logLikelihood": e.logLikelihood} for e in self],
        }
    ...
    def dummy(program, logLikelihood=0.0, logPrior=0.0, tp=None):
        ...
        t = Task("<dummy %d: %s>" % (Frontier.DUMMYFRONTIERCOUNTER, str(program)), tp, [])
        f = Frontier([FrontierEntry(program=program, logLikelihood=logLikelihood, logPrior=logPrior)], task=t)
    ...
    def temperature(self, T):
        """Divides prior by T"""
        return Frontier(
            [FrontierEntry(program=e.program, logPrior=e.logPrior / T, logLikelihood=e.logLikelihood) for e in self],
            task=self.task,
        )
    ...
    def topK(self, k):
        ...
        if k < 0:
            return self
        newEntries = sorted(self.entries, key=lambda e: (-e.logPosterior, str(e.program)))
    ...
    def replaceWithSupervised(self, g):
        ...
        return g.rescoreFrontier(
            Frontier([FrontierEntry(self.task.supervision, logLikelihood=0.0, logPrior=0.0)], task=self.task)
        )
    ...
    def summarizeFull(self):
        if self.empty:
            return "MISS " + self.task.name
        return "\n".join([self.task.name] + ["%f\t%s" % (e.logPosterior, e.program) for e in self.normalize()])
            ...
            if p in x:
                ...
                if p in y:
                    ...
                    if abs(e1.logLikelihood - e2.logLikelihood) > tolerance:
                        ...
                        eprint("WARNING: Log likelihoods deferred for %s: %f & %f" % (p, e1.logLikelihood, e2.logLikelihood))
                        ...
                        e1 = FrontierEntry(
                            program=e1.program, logLikelihood=(e1.logLikelihood + e2.logLikelihood) / 2, logPrior=e1.logPrior
                        )
            else:
                e1 = y[p]
            union.append(e1)
