...
class ContextualGrammarNetwork_Mask(nn.Module):
    ...
    def transitionMatrix(self, x):
        if len(x.shape) == 1:  # not batched
            ...
            return self._logProductions(x).unsqueeze(1).repeat(1, self.n_grammars, 1) + self._transitionMatrix.unsqueeze(
                0
            ).repeat(x.size(0), 1, 1)
        else:
...
class RecognitionModel(nn.Module):
    ...
    def taskHiddenStates(self, tasks):
        return {task: self._MLP(self.featureExtractor.featuresOfTask(task)).view(-1).data.cpu().numpy() for task in tasks}
    ...
    def train(
        ...
        def updateHelmholtzTasks():
            updateCPUs = (
                CPUs
                if hasattr(self.featureExtractor, "parallelTaskOfProgram") and self.featureExtractor.parallelTaskOfProgram
                else 1
            )
            if updateCPUs > 1:
                eprint("Updating Helmholtz tasks with", updateCPUs, "CPUs", "while using", getThisMemoryUsage(), "memory")
            if randomHelmholtz:
            ...
            if self.featureExtractor.recomputeTasks:
                for hi in range(max(0, helmholtzIndex[0] - helmholtzBatch, min(helmholtzIndex[0], len(helmholtzFrontiers)))):
            ...
            if hasattr(self.featureExtractor, "tasksOfPrograms"):
                ...
                newTasks = [
                    hf.calculateTask() for hf in helmholtzFrontiers[helmholtzIndex[0] : helmholtzIndex[0] + helmholtzBatch]
                ]
...
class RecurrentFeatureExtractor(nn.Module):
    def __init__(
        ...
        self.argumentsWithType = {
            tp: [x for t in tasks for xs, _ in t.examples for tpp, x in zip(t.request.functionArguments(), xs) if tpp == tp]
