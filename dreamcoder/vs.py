...
class VersionTable:
    ...
    def _substitutions(self, j, n):
        ...
        if self.debug:
            assert set(self.extract(s)) == set(
                e.shift(-n) for e in self.extract(j) if all(f >= n for f in e.freeVariables())
        ...
        if l.isPrimitive or l.isInvented:
            ...
            m[(self.universe, t0) if self.typed else self.universe] = j if l.i < n else self.index(l.i + 1)
        elif l.isAbstraction:
                ...
                for v2, x in xm.items():
                    ...
                    if self.typed:
                        if self.infer(a) == self.bottom:
                            continue
                        try:
                            nType = canonicalUnification(nType1, nType2, self.infer(a)[0].get(n, t0))
    ...
    def inversion(self, j):
        i = self.union([self.apply(self.abstract(b), v) for v, b in self.substitutions(j) if v != self.universe])
    ...
    def bestInventions(self, versions, bs=25):
        ...
        with timing("calculated candidates from version space"):
            ...
            candidateCost = {k: len(set(next(self.extract(k)).freeVariables())) + 1 for k in candidates}
        ...
        class B:
            def __init__(self, j):
                ...
                self.relativeCost = {
                    inhabitant: candidateCost[inhabitant] for inhabitant in inhabitants if inhabitant in candidates
                }
            ...
            def restrict(self):
                if len(self.relativeCost) > bs:
                    self.relativeCost = dict(sorted(self.relativeCost.items(), key=lambda rk: rk[1])[:bs])
                if len(self.relativeFunctionCost) > bs:
                    self.relativeFunctionCost = dict(sorted(self.relativeFunctionCost.items(), key=lambda rk: rk[1])[:bs])
            ...
            def relaxFunction(self, given, cost):
                self.relativeFunctionCost[given] = min(cost, self.getFunctionCost(given))
    ...
    def addInventionToGrammar(self, candidate, g0, frontiers, pseudoCounts=1.0):
        ...
        spaces = [self.superCache[self.incorporate(program)] for program in rewriteMapping]
        rewriteMapping = dict(zip(rewriteMapping, self.rewriteWithInvention(candidate, spaces)))
        ...
        frontiers = [
            Frontier(
                [
                    FrontierEntry(
                        program=tryRewrite(e.program, request=f.task.request), logLikelihood=e.logLikelihood, logPrior=0.0
                    )
                    for e in f
                ],
                f.task,
        ...
        # print()
        # print()
        g = Grammar.uniform([invention] + g0.primitives, continuationType=g0.continuationType).insideOutside(
            frontiers, pseudoCounts=pseudoCounts
        )
...
def induceGrammar_Beta(g0, frontiers, _=None, pseudoCounts=1.0, a=3, aic=1.0, topK=2, topI=50, structurePenalty=1.0, CPUs=1):
    ...
    while True:
        ...
        with timing("constructed %d-step version spaces" % arity):
            versions = [[v.superVersionSpace(v.incorporate(e.program), arity) for e in f] for f in restrictedFrontiers]
        ...
        with timing("scored the candidate inventions"):
            scoredCandidates = parallelMap(
                CPUs,
                lambda candidate: (candidate, scoreCandidate(candidate, restrictedFrontiers, g0)),
                candidates,
                memorySensitive=True,
                chunksize=1,
                maxtasksperchild=1,
        ...
        newGrammar, newFrontiers = v.addInventionToGrammar(bestNew, g0, frontiers, pseudoCounts=pseudoCounts)
        eprint(
            "Improved score to",
            bestScore,
            "(dS =",
...
def testSharing(projection=2):
    ...
    if projection == 3:
        ...
        for n in range(1, 2):
            ...
            P(xs, [version_size[l, n] for l in range(L)], "purple", label=None if n > 1 else "version space")
            P(xs, [program_memory[l, n] for l in range(L)], "green", label=None if n > 1 else "no version space")
            if n > 1:
                dy = 1
        ...
        plot.xticks(
            list(xs) + [xs[-1] + 2], [str(x) if j == 0 or j == L - 1 else "" for j, x in enumerate(list(xs) + [xs[-1] + 2])]
        )
        # if not logarithmic:
        #     plot.ylim([0,100000])
...
if __name__ == "__main__":
    ...
    with timing("induced DSL"):
        induceGrammar_Beta(g0, [Frontier.dummy(p, tp=tp) for p, tp in programs], CPUs=1, a=N, structurePenalty=0.0)
