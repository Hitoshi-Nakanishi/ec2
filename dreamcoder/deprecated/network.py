...
class Network(nn.Module):
    ...
    def __init__(self, input_vocabulary, target_vocabulary, hidden_size=512, embedding_size=128, cell_type="LSTM"):
        """
        ...
        if cell_type == "GRU":
            self.input_encoder_cell = nn.GRUCell(input_size=self.v_input + 1, hidden_size=self.h_input_encoder_size, bias=True)
            self.input_encoder_init = Parameter(torch.rand(1, self.h_input_encoder_size))
            self.output_encoder_cell = nn.GRUCell(
                input_size=self.v_input + 1 + self.h_input_encoder_size, hidden_size=self.h_output_encoder_size, bias=True
            )
            self.decoder_cell = nn.GRUCell(input_size=self.v_target + 1, hidden_size=self.h_decoder_size, bias=True)
        if cell_type == "LSTM":
            self.input_encoder_cell = nn.LSTMCell(input_size=self.v_input + 1, hidden_size=self.h_input_encoder_size, bias=True)
            self.input_encoder_init = nn.ParameterList(
                [Parameter(torch.rand(1, self.h_input_encoder_size)), Parameter(torch.rand(1, self.h_input_encoder_size))]
            )
            self.output_encoder_cell = nn.LSTMCell(
                input_size=self.v_input + 1 + self.h_input_encoder_size, hidden_size=self.h_output_encoder_size, bias=True
            )
            self.output_encoder_init_c = Parameter(torch.rand(1, self.h_output_encoder_size))
            self.decoder_cell = nn.LSTMCell(input_size=self.v_target + 1, hidden_size=self.h_decoder_size, bias=True)
            self.decoder_init_c = Parameter(torch.rand(1, self.h_decoder_size))

        self.W = nn.Linear(self.h_output_encoder_size + self.h_decoder_size, self.embedding_size)
        self.V = nn.Linear(self.embedding_size, self.v_target + 1)
        self.input_A = nn.Bilinear(self.h_input_encoder_size, self.h_output_encoder_size, 1, bias=False)
        self.output_A = nn.Bilinear(self.h_output_encoder_size, self.h_decoder_size, 1, bias=False)
    ...
    def __getstate__(self):
        if hasattr(self, "opt"):
            return dict([(k, v) for k, v in self.__dict__.items() if k is not "opt"] + [("optstate", self.opt.state_dict())])
    ...
    def output_encoder_get_init(self, input_encoder_h):
        ...
        if self.cell_type == "LSTM":
            return (input_encoder_h, self.output_encoder_init_c.repeat(input_encoder_h.size(0), 1))

    def decoder_get_init(self, output_encoder_h):
        if self.cell_type == "GRU":
            return output_encoder_h
        if self.cell_type == "LSTM":
            return (output_encoder_h, self.decoder_init_c.repeat(output_encoder_h.size(0), 1))
    ...
    def run(self, inputs, outputs, target=None, mode="sample"):
        ...
        inputs_scatter = [
            Variable(torch.zeros(max_length_input[j], batch_size, self.v_input + 1).scatter_(2, inputs[j][:, :, None], 1))
            for j in range(n_examples)
        ]  # n_examples * (max_length_input * batch_size * v_input+1)
        outputs_scatter = [
            Variable(torch.zeros(max_length_output[j], batch_size, self.v_input + 1).scatter_(2, outputs[j][:, :, None], 1))
            for j in range(n_examples)
        ]  # n_examples * (max_length_output * batch_size * v_input+1)
        if target is not None:
            target_scatter = Variable(
                torch.zeros(max_length_target, batch_size, self.v_target + 1).scatter_(2, target[:, :, None], 1)
        ...
        for j in range(n_examples):
            ...
            embedding = input_H[j].gather(0, Variable(embedding_idx[None, :, None].repeat(1, 1, self.h_input_encoder_size)))[0]
        ...
        def input_attend(j, h_out):
            ...
            scores = (
                self.input_A(
                    input_H[j].view(max_length_input[j] * batch_size, self.h_input_encoder_size),
                    h_out.view(batch_size, self.h_output_encoder_size).repeat(max_length_input[j], 1),
        ...
        for j in range(n_examples):
            ...
            for i in range(max_length_output[j]):
                state = self.output_encoder_cell(torch.cat([outputs_scatter[j][i, :, :], input_attend(j, h)], 1), state)
            ...
            embedding_idx = active.sum(0).long() - 1
            embedding = output_H[j].gather(0, Variable(embedding_idx[None, :, None].repeat(1, 1, self.h_output_encoder_size)))[
                0
            ]
        ...
        def output_attend(j, h_dec):
            ...
            scores = (
                self.output_A(
                    output_H[j].view(max_length_output[j] * batch_size, self.h_output_encoder_size),
                    h_dec.view(batch_size, self.h_decoder_size).repeat(max_length_output[j], 1),
        ...
        target = target if mode == "score" else torch.zeros(max_length_target, batch_size).long()
        decoder_states = [self.decoder_get_init(output_embeddings[j]) for j in range(n_examples)]  # P
            ...
            for j in range(n_examples):
                if mode == "score":
                    ...
                    target_char_scatter = Variable(
                        torch.zeros(batch_size, self.v_target + 1).scatter_(1, target[i, :, None], 1)
                    )
    ...
    def inputsToTensors(self, inputss):
        ...
        for j in range(len(inputss[0])):
            ...
            for i in range(len(inputs)):
                s = inputs[i]
                if len(s) > 0:
                    t[: len(s), i] = torch.LongTensor([self.input_vocabulary.index(x) for x in s])
            tensors.append(t)
        return tensors
    ...
    def targetToTensor(self, targets):
        ...
        t = torch.ones(1 if maxlen == 0 else maxlen + 1, len(targets)).long() * self.v_target
        for i in range(len(targets)):
            s = targets[i]
            if len(s) > 0:
                t[: len(s), i] = torch.LongTensor([self.target_vocabulary.index(x) for x in s])
        return t
