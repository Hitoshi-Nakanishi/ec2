...
class Grammar(object):
    ...
    def fromProductions(productions, logVariable=0.0, continuationType=None):
        ...
        return Grammar(logVariable, [(l, p.infer(), p) for l, p in productions], continuationType=continuationType)
    ...
    def _sample(self, request, context, environment, maximumDepth):
        if request.isArrow():
            context, expression = self._sample(
                request.arguments[1], context, [request.arguments[0]] + environment, maximumDepth
            )
    ...
    def likelihoodSummary(self, context, environment, request, expression, silent=False):
        if request.isArrow():
            ...
            return self.likelihoodSummary(
                context, [request.arguments[0]] + environment, request.arguments[1], expression.body, silent=silent
            )
    ...
    def bestFirstEnumeration(self, request):
        ...
        def g(parentCost, request, _=None, context=None, environment=[], k=None):
            ...
            if request.isArrow():
                ...
                candidates = self.buildCandidates(
                    request, context, environment, normalize=True, returnProbabilities=False, returnTable=True
                )
                choices(
                    parentCost,
                    [
            ...
            if argumentTypes == []:
                ...
                g(
                    ...
                    k=lambda newCost, newContext, argument: ga(
                        newCost, Application(f, argument), argumentTypes[1:], context=newContext, environment=environment, k=k
                    ),
                )
    ...
    def rescoreFrontier(self, frontier):
        return Frontier(
            [
                FrontierEntry(
                    e.program, logPrior=self.logLikelihood(frontier.task.request, e.program), logLikelihood=e.logLikelihood
                )
                for e in frontier
            ],
            frontier.task,
    ...
    def insideOutside(self, frontiers, pseudoCounts, iterations=1):
        ...
        frontiers = [
            Frontier(
                [
                    FrontierEntry(
                        (summary, summary.toUses()), logPrior=summary.logLikelihood(self), logLikelihood=e.logLikelihood
                    )
                    for e in f
        ...
        for i in range(iterations):
            ...
            g = Grammar(
                ...
                [
                    (
                        math.log(u.actualUses.get(p, 0.0) + pseudoCounts) - math.log(u.possibleUses.get(p, 0.0) + pseudoCounts),
                        t,
                        p,
                    )
            ...
            if i < iterations - 1:
                frontiers = [
                    Frontier(
                        [
                            FrontierEntry((summary, uses), logPrior=summary.logLikelihood(g), logLikelihood=e.logLikelihood)
                            for e in f
    ...
    def enumerateApplication(
        ...
        if argumentRequests == []:
            ...
            for argL, newContext, arg in self.enumeration(
                context, environment, argRequest, upperBound=upperBound, lowerBound=0.0, maximumDepth=maximumDepth
            ):
    ...
    def sketchEnumeration(self, context, environment, request, sk, upperBound, maximumDepth=20, lowerBound=0.0):
        ...
        if sk.isHole:
            yield from self.enumeration(
                context, environment, request, upperBound, maximumDepth=maximumDepth, lowerBound=lowerBound
            )
    ...
    def sketchApplication(
        ...
        if argumentRequests == []:
            ...
            for argL, newContext, arg in self.sketchEnumeration(
                context, environment, argRequest, firstSketch, upperBound=upperBound, lowerBound=0.0, maximumDepth=maximumDepth
            ):
    ...
    def sketchLogLikelihood(self, request, full, sk, context=Context.EMPTY, environment=[]):
        ...
        if sk.isHole:
            ...
            if summary is None:
                eprint("FATAL: program [ %s ] does not have a likelihood summary." % full, "r = ", request, "\n", self)
                assert False
            ...
            return self.sketchLogLikelihood(
                request.arguments[1], full.body, sk.body, context=context, environment=[v] + environment
            )
    ...
    def sketchllApplication(
        self, context, environment, sk_function, sk_arguments, full_function, full_arguments, argumentRequests
    ):
        if argumentRequests == []:
            ...
            argL, newContext = self.sketchLogLikelihood(
                argRequest, full_firstSketch, sk_firstSketch, context=context, environment=environment
            )
            ...
            resultL, context = self.sketchllApplication(
                newContext, environment, sk_newFunction, sk_laterSketches, full_newFunction, full_laterSketches, laterRequests
            )
...
normalizers = {%s})""" % (
    ...
    def denominator(self, grammar):
        return sum(count * lse([grammar.expression2likelihood[p] for p in ps]) for ps, count in self.normalizers.items())
...
class ContextualGrammar:
    ...
    def __str__(self):
        lines = ["No parent:", str(self.noParent), "", "Variable parent:", str(self.variableParent), "", ""]
    ...
    def json(self):
        return {
            ...
            "productions": [{"program": str(e), "arguments": [gp.json() for gp in gs]} for e, gs in self.library.items()],
        }
    ...
    def likelihoodSummary(self, parent, parentIndex, context, environment, request, expression):
        if request.isArrow():
            ...
            return self.likelihoodSummary(
                parent, parentIndex, context, [request.arguments[0]] + environment, request.arguments[1], expression.body
            )
        if parent is None:
            g = self.noParent
        elif parent.isIndex:
        ...
        thisSummary.record(parent, parentIndex, f, possibles, constant=-math.log(numberOfVariables) if f.isIndex else 0)
    ...
    def _sample(self, parent, parentIndex, context, environment, request, maximumDepth):
        if request.isArrow():
            context, body = self._sample(
                parent, parentIndex, context, [request.arguments[0]] + environment, request.arguments[1], maximumDepth
            )
        ...
        candidates = g.buildCandidates(
            request, context, environment, normalize=True, returnProbabilities=True, mustBeLeaf=(maximumDepth <= 1)
        )
        newType, chosenPrimitive, context = sampleDistribution(candidates)
    ...
    def expectedUsesMonteCarlo(self, request, debug=None):
        ...
        primitive2index = {primitive: i for i, primitive in enumerate(primitives) if primitive.isInvented or noInventions}
    ...
    def enumeration(
        self, context, environment, request, upperBound, parent=None, parentIndex=None, maximumDepth=20, lowerBound=0.0
    ):
        """Enumerates all programs whose MDL satisfies: lowerBound <= MDL < upperBound"""
        if upperBound < 0 or maximumDepth == 1:
            return
...
def batchLikelihood(jobs):
    ...
    superGrammar = Grammar.uniform(
        list({p for _1, _2, g in jobs for p in g.primitives}), continuationType=list(jobs)[0][-1].continuationType
    )
